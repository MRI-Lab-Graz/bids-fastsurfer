#!/usr/bin/env python3
"""
Analyze FreeSurfer Qdec files - Run statistical tables, surface preprocessing, and QC.

Supports both cross-sectional and longitudinal studies with automatic detection:
  - Cross-sectional: Single timepoint per subject
  - Longitudinal: Multiple timepoints per subject with .long directories

This script takes a Qdec file and performs advanced analysis steps:
  - asegstats2table and aparcstats2table for volumetric data
  - Surface preprocessing (mris_preproc + smoothing)
  - Quality control with fsqc
  - .long symlink verification (for longitudinal)

Can optionally generate the Qdec file internally if --participants is provided,
but typically you would first use generate_qdec.py and then analyze with this script.

Usage examples:

  1. Analyze existing Qdec (auto-detects cross-sectional vs longitudinal):
     python scripts/analyse_qdec.py \
  ...
  2. Full analysis with all features:
     python scripts/analyse_qdec.py \
       ...
       --subjects-dir data/subjects \
       --output results/ \
       --aseg --aparc --surf --qc --qc-screenshots

  3. Generate Qdec + analyze in one step:
     python scripts/analyse_qdec.py \
  ...
  4. Force analysis mode (override auto-detection):
     python scripts/analyse_qdec.py \
       --qdec results/qdec.table.dat \
       --subjects-dir data/subjects \
       --type longitudinal \
  ...
  5. Use config file for settings:
     python scripts/analyse_qdec.py \
       --config configs/analyse.json

Notes:
- Study type (cross-sectional/longitudinal) is auto-detected from Qdec structure
- Use --type to override auto-detection if needed
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    p = argparse.ArgumentParser(
        description="Analyze FreeSurfer Qdec files: statistical tables, surface prep, and QC",
        parents=[p0],
    )

    # Primary input arguments
    input_group = p.add_argument_group("Primary inputs")
    input_group.add_argument(
        "--qdec",
        type=Path,
        default=None,
        help="Path to existing Qdec file to analyze (preferred workflow). "
        "If omitted, will generate Qdec from --participants",
    )
    input_group.add_argument(
        "--subjects-dir",
        required=True,
    ...
    input_group.add_argument(
        "--type",
        choices=["auto", "cross-sectional", "longitudinal"],
        default="auto",
        help="Study type (default: auto-detect from Qdec structure). "
        "Use 'cross-sectional' for single timepoint, 'longitudinal' for multiple timepoints per subject.",
    )

    # Optional Qdec generation (if --qdec not provided)
    qdec_gen_group = p.add_argument_group(
        "Qdec generation (only if --qdec not provided)",
        "These options are used to generate a Qdec file if you don't provide one. "
        "For more control, use generate_qdec.py first, then analyze with this script.",
    )
    qdec_gen_group.add_argument(
        "--participants", type=Path, default=None, help="Path to BIDS participants.tsv"
    )
...
def run_asegstats2table(
    ...
    Args:
        ...
        study_type: 'cross-sectional' or 'longitudinal' - determines command flags
    """
    ...
    try:
        ...
        if study_type == "longitudinal" and (
            ...
            print(
                f"asegstats2table failed with exit code {exc.returncode}. Command: {' '.join(cmd[:10])}...",
                file=sys.stderr,
            )
            if error_output:
...
def run_surf_mass_univariate(
    ...
    # Only applicable for longitudinal studies
    # Skip auto-linking if in dry-run mode
    if study_type == "longitudinal" and not dry_run:
        try:
    ...
    elif study_type == "longitudinal" and dry_run:
    ...
    for hemi in hemis:
        for meas in measures:
            ...
            try:
                ...
                print(
                    f"aparcstats2table failed with exit code {exc.returncode}. Command: {' '.join(cmd[:10])}...",
                    file=sys.stderr,
                )
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if qdec_provided:
        ...
        if not args.qdec.exists():
            print(f"ERROR: Qdec file not found: {args.qdec}", file=sys.stderr)
            return 2
        out_path = args.qdec
        ...
        if args.type == "auto":
            ...
            print(f"[INFO] Detected study type: {detected_type}")
        ...
        if not args.participants:
            print(
                "ERROR: Either --qdec or --participants must be provided.\n"
                "Preferred: Use generate_qdec.py first, then analyze with --qdec",
                file=sys.stderr,
            )
            return 2
        ...
        # Resolve --output: treat as directory by default
        ...
        try:
            ...
            print(f"[WARN] Could not determine output type: {e}", file=sys.stderr)
            out_path = out_root / qdec_filename
        ...
        if args.type == "auto":
            ...
            print(f"[INFO] Detected study type: {study_type}")
        else:
            study_type = args.type
            print(f"[INFO] Using specified study type: {study_type}")
    ...
    # Only applicable for longitudinal studies
    if study_type == "longitudinal" and (args.verify_long or args.link_long):
        verify_and_link_long(
            subj_dir,
            timepoints,
            link=args.link_long,
    ...
    elif study_type == "cross-sectional" and (args.verify_long or args.link_long):
        print(
            "[INFO] Skipping .long symlink verification (not applicable for cross-sectional studies)."
        )
    ...
    if args.aseg:
        if args.link_dry_run:
            ...
            rc = run_asegstats2table(out_path, subj_dir, study_type=study_type)
            if rc != 0:
                return rc
    if args.aparc:
        if args.link_dry_run:
