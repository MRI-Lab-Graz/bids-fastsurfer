#!/usr/bin/env python3
"""
Extract longitudinal volumes for the remaining FreeSurfer/FastSurfer aseg
structures not covered by extract_basal_ganglia.py / extract_hippo_subfields.py
/ extract_amygdala_subfields.py: thalamus, ventral DC, brain-stem,
cerebellum (cortex + white matter), and the five corpus callosum segments.

Deliberately excludes ventricles, CSF, choroid plexus, vessels, WM/non-WM
hypointensities, and the optic chiasm -- artifact/nuisance categories in
standard aseg output, not neuroanatomical regions of scientific interest
for a plasticity analysis.

Emits the same two output formats as the other extractors:
...
# struct_name -> (hemisphere or None for midline, roi label)
STRUCTURES = {
    "Left-Thalamus": ("lh", "Thalamus"),
    "Right-Thalamus": ("rh", "Thalamus"),
    "Left-VentralDC": ("lh", "VentralDC"),
    "Right-VentralDC": ("rh", "VentralDC"),
    "Left-Cerebellum-Cortex": ("lh", "Cerebellum_Cortex"),
    "Right-Cerebellum-Cortex": ("rh", "Cerebellum_Cortex"),
    "Left-Cerebellum-White-Matter": ("lh", "Cerebellum_WM"),
    "Right-Cerebellum-White-Matter": ("rh", "Cerebellum_WM"),
    "Brain-Stem": (None, "Brain_Stem"),
    "CC_Posterior": (None, "CC_Posterior"),
    "CC_Mid_Posterior": (None, "CC_Mid_Posterior"),
    "CC_Central": (None, "CC_Central"),
    "CC_Mid_Anterior": (None, "CC_Mid_Anterior"),
    "CC_Anterior": (None, "CC_Anterior"),
}
...
def main() -> None:
    ...
    p.add_argument("--long-table-name", default="other_regions.long.table")
    p.add_argument("--tidy-name", default="other_regions_tidy.tsv")
    p.add_argument("--missing-log", default="missing_other_regions_subjects.tsv")
    args = p.parse_args()
    ...
    for subject_base, session, d in timepoints:
        ...
        if not stats_file.is_file():
            missing.append({"subject_id": subject_base, "session": session, "reason": "aseg.stats not found"})
            continue
        ...
        wide_row: Dict[str, object] = {"Measure.volume": fsid_long, "fsid": fsid, "fsid_base": subject_base, "eTIV": etiv}

        for struct_name, vol in volumes.items():
            hemi, roi = STRUCTURES[struct_name]
            col = f"{hemi}_{roi}" if hemi else roi
            wide_row[col] = vol
            tidy_rows.append({
                "subject_id": subject_base,
                "session": session,
                "fsid": fsid,
                "hemisphere": hemi if hemi else "midline",
                "roi": roi,
                "volume": vol,
                "etiv": etiv,
            })
    ...
    if not wide_rows:
        print("No complete subject/session outputs found.", file=sys.stderr)
        sys.exit(1)

    long_table_path = outdir / args.long_table_name
    fixed_cols = ["Measure.volume", "fsid", "fsid_base", "eTIV"]
    data_cols = sorted({f"{hemi}_{roi}" if hemi else roi for hemi, roi in STRUCTURES.values()})
    ...
    if missing:
        ...
        with open(missing_path, "w", newline="") as f:
            ...
            w.writeheader()
            w.writerows(missing)
        print(f"Wrote {missing_path} ({len(missing)} subject/session timepoints missing output)", file=sys.stderr)
    else:
