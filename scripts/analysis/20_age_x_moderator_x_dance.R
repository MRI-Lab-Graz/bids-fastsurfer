#!/usr/bin/env Rscript
#
# Does the age x dance moderation finding (08_moderator_analysis.R: age's
# relationship to CA3/CA4/dentate-gyrus change is stronger in the running
# group) itself depend on sex, or on a psychological trait/state (ADS
# depression, PANAS affect, STAI anxiety, FSozU social support, TSDZ)?
# Tests a three-way dance x age_z x third_var interaction for each
# candidate third variable, across the pre-specified ROI set.
#
# Sex is always tested (a three-level model would be underpowered with a
# four-way age x sex x survey x dance term, so sex and each survey
# moderator are tested as SEPARATE three-way models, not jointly).
#
# LOSO runs only on hits below --loso-screen-threshold, mirroring
# 08_moderator_analysis.R's triage logic -- this is exploratory
# (post-hoc, conditional on the age x dance finding already existing), so
# treat any hit as motivation for a dedicated follow-up, not a finding on
# its own, until LOSO-checked.
...
option_list <- list(
  make_option(c("-t", "--tidy"), type="character",
              help="Path to a tidy volumes TSV"),
  ...
  make_option(c("--extra-moderator-cols"), type="character", default=NULL,
              help="Comma-separated column names in --moderators to test as third variables (e.g. 'ads_score,panas_pa,panas_na,tsdz_total,fsozu_mean,stai_total')"),
  make_option(c("-o", "--outdir"), type="character", default="results/20_age_x_moderator_x_dance",
...
fit_three_way <- function(change_dat, third_term) {
  f_str <- paste0("log_change ~ dance * age_z * ", third_term,
                   " + log_baseline_z + hemisphere + followup_f + etiv_z + (1 | subject_id)")
  ...
  interaction_term <- paste0("dance:age_z:", third_term)
  if (!is.null(at) && interaction_term %in% rownames(at)) at[interaction_term, "Pr(>F)"] else NA_real_
}

msg("Testing %d third variables x %d ROIs for dance:age_z:third_var (exploratory)...\n", length(third_var_specs), length(roi_set))
...
if (!is.null(screen_hits) && nrow(screen_hits)) {
  msg("\nRunning targeted LOSO for %d (third_var, ROI) hits with uncorrected p < %.2f...\n", nrow(screen_hits), opt$`loso-screen-threshold`)
  loso_rows <- list()
  ...
  cat(sprintf("Targeted LOSO for (third_var, ROI) hits with uncorrected p < %.2f\n\n", opt$`loso-screen-threshold`), file=con)
  for (i in seq_len(nrow(screen_hits))) {
    tv_name <- screen_hits$third_var[i]; roi_name <- screen_hits$roi[i]
  ...
  msg("\nNo (third_var, ROI) combination had uncorrected p < %.2f; nothing to LOSO-check.\n", opt$`loso-screen-threshold`)
}
