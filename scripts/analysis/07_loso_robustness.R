...
# dance_main effect (the clean intervention test)
# ------------------------------------------------------------------------
base_formula <- log_change ~ dance * followup_f + log_baseline_z + hemisphere + age_z + sex + etiv_z + (1 | subject_id)
...
for (roi_name in unique(agg$subfield)) {
  ...
  full_p <- if (!is.null(full_fit)) {
    ...
    if (!is.null(at) && "dance" %in% rownames(at)) at["dance", "Pr(>F)"] else NA_real_
  } else NA_real_

  for (excl_subj in all_subjects) {
    d_sub <- change_dat_full[change_dat_full$subject_id != excl_subj, , drop=FALSE]
    if (!length(unique(d_sub$dance)) == 2 || nrow(d_sub) < 10) next
    ...
    if (!is.null(fit)) {
      ...
      if (!is.null(at) && "dance" %in% rownames(at)) p_val <- at["dance", "Pr(>F)"]
      fe <- tryCatch(lme4::fixef(fit), error=function(e) NULL)
      if (!is.null(fe) && "dancedance" %in% names(fe)) est <- fe[["dancedance"]]
    }
    loso_rows[[length(loso_rows) + 1]] <- data.frame(
      roi = roi_name, excluded_subject = excl_subj,
      dance_main_p = p_val, dance_main_estimate = est,
      full_data_p = full_p,
...
flip_rows <- loso_df[!is.na(loso_df$dance_main_p) & !is.na(loso_df$full_data_p) &
                      (loso_df$dance_main_p < opt$alpha) != (loso_df$full_data_p < opt$alpha), , drop=FALSE]
...
influence_summary <- do.call(rbind, lapply(split(loso_df, loso_df$roi), function(g) {
  g <- g[!is.na(g$dance_main_p), , drop=FALSE]
  if (!nrow(g)) return(NULL)
  most_influential_idx <- which.max(abs(g$dance_main_p - g$full_data_p[1]))
  data.frame(
    roi = g$roi[1],
    full_data_p = g$full_data_p[1],
    loso_p_min = min(g$dance_main_p), loso_p_max = max(g$dance_main_p),
    most_influential_subject = g$excluded_subject[most_influential_idx],
    most_influential_p_when_excluded = g$dance_main_p[most_influential_idx],
    stringsAsFactors = FALSE
  )
}))
...
manova_dat <- manova_dat[manova_dat$group %in% c(dance_groups, control_group), , drop=FALSE]
manova_dat$dance <- factor(ifelse(manova_dat$group %in% dance_groups, "dance", "control"), levels=c("control","dance"))
manova_dat$sex <- factor(manova_dat$sex)
manova_dat$age_z <- as.numeric(scale(manova_dat$age))
manova_dat <- manova_dat[complete.cases(manova_dat[, c(change_cols,"dance","age_z","sex")]), , drop=FALSE]

run_manova_p <- function(d) {
  if (length(unique(d$dance)) < 2 || nrow(d) < length(change_cols) + 5) return(NA_real_)
  Y <- as.matrix(d[, change_cols])
  fit <- tryCatch(manova(Y ~ dance + age_z + sex, data=d), error=function(e) NULL)
  if (is.null(fit)) return(NA_real_)
  s <- tryCatch(as.data.frame(summary(fit, test="Pillai")$stats), error=function(e) NULL)
  if (is.null(s) || !"dance" %in% rownames(s)) return(NA_real_)
  s["dance", "Pr(>F)"]
}
...
for (excl_subj in unique(manova_dat$subject_id)) {
  ...
  manova_loso_rows[[excl_subj]] <- data.frame(
    ...
    manova_dance_p = run_manova_p(d_sub),
...
manova_flips <- manova_loso_df[!is.na(manova_loso_df$manova_dance_p) &
                                (manova_loso_df$manova_dance_p < opt$alpha) != (manova_loso_df$full_data_p < opt$alpha), , drop=FALSE]
...
cat(sprintf(
  ...
  full_manova_p, min(manova_loso_df$manova_dance_p, na.rm=TRUE), max(manova_loso_df$manova_dance_p, na.rm=TRUE),
