#!/usr/bin/env Rscript
#
# Basal ganglia analysis: same confirmatory LMM design as 01_primary_lmm.R /
# 10_amygdala_lmm.R, applied to Caudate, Putamen, Pallidum, Accumbens (from
# extract_basal_ganglia.py). This ROI set comes from the BASE FastSurfer
# recon pipeline (aseg.stats), not the specialized subfield segmentation --
# coverage here is typically much more complete than hippocampus/amygdala.
#
...
option_list <- list(
  make_option(c("-t", "--tidy"), type="character",
              help="Path to basal_ganglia_tidy.tsv (from extract_basal_ganglia.py)"),
  make_option(c("-p", "--participants"), type="character",
              help="Path to participants TSV with columns: subject_id, group, age, sex"),
  make_option(c("-o", "--outdir"), type="character", default="results/11_basal_ganglia_lmm",
...
msg("Loading tidy basal ganglia data from %s...\n", opt$tidy)
...
summary_rows <- list()
for (roi_name in unique(agg$roi)) {
  msg("Fitting model for ROI: %s\n", roi_name)
  d_roi <- agg[agg$roi == roi_name, , drop=FALSE]
...
if (!is.null(summary_df) && nrow(summary_df)) {
  ...
  write.csv(summary_df, file.path(opt$outdir, "basal_ganglia_summary.csv"), row.names=FALSE)
  msg("\nWrote combined summary (FDR-corrected across %d ROIs) to basal_ganglia_summary.csv\n", nrow(summary_df))
} else {
