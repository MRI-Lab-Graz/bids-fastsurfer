#!/usr/bin/env Rscript
#
# Thalamic nucleus analysis: same confirmatory LMM design as
# 01_primary_lmm.R / 10_amygdala_lmm.R, applied to thalamic nuclei (from
# FreeSurfer 8.2.0's `segment_subregions thalamus --long-base`, the same
# family of subregion tools as the hippo-amygdala run but a separate
# invocation on top of the aseg/norm outputs).
#
# No pre-registered a priori nucleus subset exists for this ROI set, so all
# nuclei are tested (FDR-corrected across them), with Whole_thalamus reported
# alongside the individual nuclei as the summary measure.
#
...
option_list <- list(
  make_option(c("-t", "--tidy"), type="character",
              help="Path to thalamic_tidy.tsv (nucleus, hemisphere, volume, etiv columns)"),
  make_option(c("-p", "--participants"), type="character",
              help="Path to participants TSV with columns: subject_id, group, age, sex"),
  make_option(c("-o", "--outdir"), type="character", default="results/24_thalamic_lmm",
...
msg("Loading tidy thalamic data from %s...\n", opt$tidy)
...
if (!is.null(summary_df) && nrow(summary_df)) {
  ...
  write.csv(summary_df, file.path(opt$outdir, "thalamic_summary.csv"), row.names=FALSE)
  msg("\nWrote combined summary (FDR-corrected across %d nuclei) to thalamic_summary.csv\n", nrow(summary_df))
} else {
